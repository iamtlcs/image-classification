...
class AlexNet(keras.Model):
    def __init__(self, input_shape = (224, 224, 3), activ_Conv2D = 'relu', activ_dense = 'relu', activ_output = 'softmax', droprate = 0.5, num_classes = 8, **kwargs):
        ...
        # Block 1
        self.input_Conv2D = keras.layers.Conv2D(filters=96, kernel_size=(11,11), strides=(4,4), activation=activ_Conv2D, input_shape=input_shape, padding="same", name="Block1_Conv")
        self.batchnorm1 = keras.layers.BatchNormalization()
        self.MaxPool2D1 = keras.layers.MaxPool2D(pool_size=(3,3), strides=(2,2), name="Block1_pool")
        # Block 2
        self.Conv2D1 = keras.layers.Conv2D(filters=256, kernel_size=(5,5), strides=(1,1), activation=activ_Conv2D, padding="same", name="Block2_Conv")
        ...
        # Block 3
        self.Conv2D2 = keras.layers.Conv2D(filters=384, kernel_size=(3,3), strides=(1,1), activation=activ_Conv2D, padding="same", name="Block3_Conv1")
        self.batchnorm3 = keras.layers.BatchNormalization()
        self.Conv2D3 = keras.layers.Conv2D(filters=384, kernel_size=(3,3), strides=(1,1), activation=activ_Conv2D, padding="same", name="Block3_Conv2")
        self.batchnorm4 = keras.layers.BatchNormalization()
        self.Conv2D4 = keras.layers.Conv2D(filters=256, kernel_size=(3,3), strides=(1,1), activation=activ_Conv2D, padding="same", name="Block3_Conv3")
